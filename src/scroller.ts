...
export class OverScroller {
  ...
  getCurrVelocity(): number {
    let squaredNorm =
      this.mScrollerX.mCurrVelocity * this.mScrollerX.mCurrVelocity;
    squaredNorm +=
      this.mScrollerY.mCurrVelocity * this.mScrollerY.mCurrVelocity;
  ...
  startScroll(
    ...
    dx: number,
    dy: number,
    duration = OverScroller.DEFAULT_DURATION
  ) {
  ...
  fling(
    ...
    if (this.mFlywheel && !this.isFinished()) {
      ...
      if (
        Math_signum(velocityX) == Math_signum(oldVelocityX) &&
  ...
  timePassed(): number {
    ...
    const startTime = Math.min(
      this.mScrollerX.mStartTime,
      this.mScrollerY.mStartTime
    );
  ...
  isScrollingInDirection(xvel: number, yvel: number): boolean {
    ...
    return (
      !this.isFinished() &&
      Math_signum(xvel) == Math_signum(dx) &&
...
class SplineOverScroller {
  ...
  static P2 =
    1.0 - SplineOverScroller.END_TENSION * (1 - SplineOverScroller.INFLEXION);

  static NB_SAMPLES = 100;
  static SPLINE_POSITION = new Array(SplineOverScroller.NB_SAMPLES + 1);
  static SPLINE_TIME = new Array(SplineOverScroller.NB_SAMPLES + 1);
  ...
  static _staticFunc = (function () {
    ...
    for (let i = 0; i < SplineOverScroller.NB_SAMPLES; i++) {
      ...
      while (true) {
        ...
        tx =
          coef *
            ((1.0 - x) * SplineOverScroller.P1 + x * SplineOverScroller.P2) +
          x * x * x;
        if (Math.abs(tx - alpha) < 1e-5) break;
        if (tx > alpha) x_max = x;
        else x_min = x;
      }
      SplineOverScroller.SPLINE_POSITION[i] =
        coef * ((1.0 - x) * SplineOverScroller.START_TENSION + x) + x * x * x;
      ...
      while (true) {
        ...
        coef = 3.0 * y * (1.0 - y);
        dy =
          coef * ((1.0 - y) * SplineOverScroller.START_TENSION + y) + y * y * y;
        if (Math.abs(dy - alpha) < 1e-5) break;
        if (dy > alpha) y_max = y;
        else y_min = y;
      }
      SplineOverScroller.SPLINE_TIME[i] =
        coef * ((1.0 - y) * SplineOverScroller.P1 + y * SplineOverScroller.P2) +
        y * y * y;
    }
    SplineOverScroller.SPLINE_POSITION[
      SplineOverScroller.NB_SAMPLES
    ] = SplineOverScroller.SPLINE_TIME[SplineOverScroller.NB_SAMPLES] = 1.0;
  })();
  ...
  constructor() {
    this.mFinished = true;
    let ppi = window.devicePixelRatio * 160;
    this.mPhysicalCoeff =
      9.80665 * // g (m/s^2)
      39.37 * // inch/meter
      ppi *
  ...
  updateScroll(q: number) {
    this.mCurrentPosition =
      this.mStart + Math.round(q * (this.mFinal - this.mStart));
  }
  ...
  startSpringback(start: number, end: number, velocity: number) {
    ...
    this.mDuration = Math.floor(
      1000.0 * Math.sqrt((-2.0 * (delta / density)) / this.mDeceleration)
    );
  }
  ...
  fling(
    ...
    if (velocity != 0) {
      this.mDuration = this.mSplineDuration = this.getSplineFlingDuration(
        velocity
      );
  ...
  getSplineDeceleration(velocity: number): number {
    return Math.log(
      (SplineOverScroller.INFLEXION * Math.abs(velocity)) /
  ...
  getSplineFlingDistance(velocity: number): number {
    ...
    return (
      this.mFlingFriction *
      this.mPhysicalCoeff *
      Math.exp((SplineOverScroller.DECELERATION_RATE / decelMinusOne) * l)
    );
  }
  ...
  fitOnBounceCurve(start: number, end: number, velocity: number) {
    ...
    let distanceToApex =
      (velocity * velocity) / 2.0 / Math.abs(this.mDeceleration);
  ...
  startBounceAfterEdge(start: number, end: number, velocity: number) {
    this.mDeceleration = SplineOverScroller.getDeceleration(
      velocity == 0 ? start - end : velocity
    );
  ...
  onEdgeReached() {
    ...
    let distance =
      (this.mVelocity * this.mVelocity) / (2 * Math.abs(this.mDeceleration));
    const sign = Math_signum(this.mVelocity);

    if (distance > this.mOver) {
      // Default deceleration is not sufficient to slow us down before boundary
      this.mDeceleration =
        (-sign * this.mVelocity * this.mVelocity) / (2.0 * this.mOver);
      distance = this.mOver;
    }
  ...
  update(): boolean {
    ...
    switch (this.mState) {
      case SplineOverScroller.SPLINE: {
        ...
        this.mCurrVelocity =
          ((velocityCoef * this.mSplineDistance) / this.mSplineDuration) * 1000;
        break;
      }
